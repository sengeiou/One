...
import com.ubt.factorytest.configuration.FactoryParamConf;
...
public class FactoryTool {
    ...
    private FactoryParamConf factoryParamConf = new FactoryParamConf();
    ...
    public void init(){
        pressCnt = 0;
        String param = SpUtils.getInstance().getFactoryConf();
        if(!TextUtils.isEmpty(param)){
            factoryParamConf = factoryParamConf.fromJson(param);
        }
    }
...
//                req = new PlayAction("action/my creation/" + "Action-老化测试动作简版.hts");
                req = new PlayAction("action/testmode/Action-老化测试动作简版.hts");
        ...
        if(position >= 0) {
            ...
            switch (itemID){
                case TestClickEntity.TEST_ITEM_START_TIME:
                    ...
                    dataServer.setHasTested(position);
                    break;
                case TestClickEntity.TEST_ITEM_SOFT_VERSION:
                    result = new String(data.getmParam());
                    if(factoryParamConf.getSoftVer().equalsIgnoreCase(result)){
                ...
                case TestClickEntity.TEST_ITEM_HARDWARE_VER:
                    ...
                    if(factoryParamConf.getFirmwareVer().equalsIgnoreCase(result)){
                        dataServer.setDataisPass(position, true);
                    }else{
                        dataServer.setDataisPass(position, false);
                    }
                ...
                case TestClickEntity.TEST_ITEM_ELECTRICCHARGE:
                    ...
                    if(power[3] >= factoryParamConf.getMinPower() &&
                            power[3] <= factoryParamConf.getMaxPower()){
                        dataServer.setDataisPass(position, true);
                    }else{
                        dataServer.setDataisPass(position, false);
                    }
     ...
     * @return
     */
    public int getPosition(DataServer dataServer, int itemID){
        int position;
    ...
    public void parseDevStatus(byte[] status){
        if(status[0] == 0x02){
            currentVol = status[1]&0xff;
            Log.i(TAG,"当前音量:"+currentVol);
        }
    }
