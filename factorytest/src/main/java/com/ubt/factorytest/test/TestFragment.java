...
import com.ubt.factorytest.bluetooth.netconnect.ConfirmDialog;
...
import com.ubt.factorytest.configuration.ConfFragment;
...
public class TestFragment extends SupportFragment implements TestContract.View {
    ...
    private static final int MSG_FRAGMENT_BACK = 4;
            ...
            public boolean onMenuItemClick(MenuItem item) {
                ...
                switch (id){
                    ...
                    case R.id.action_wifi_status:
                        ...
                        break;
                    case R.id.action_param_conf:
                        ConfFragment confFragment = ConfFragment.newInstance();
                        start(confFragment);
                        break;
                }
                return true;
...
//                Log.i(TAG,"DATACACHE="+mPresenter.getDataCache().toString());
            ...
            public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
                switch(view.getId()){
                    case R.id.btn_ok:
                        ...
                        adapter.notifyItemChanged(position, "123121");
                        mAdapter.getItem(position).setTested(true);*/
                        mAdapter.getItem(position).setPass(true);
                        adapter.notifyItemChanged(position, "123121");
                        mAdapter.getItem(position).setTested(true);
                        break;
                    case R.id.btn_fail:
                        mAdapter.getItem(position).setPass(false);
                        adapter.notifyItemChanged(position, "123121");
                        mAdapter.getItem(position).setTested(true);
                        break;
    ...
    public void btDisconnected() {
        ...
        if(mHandler != null) {
            ...
            mHandler.sendEmptyMessage(MSG_FRAGMENT_BACK);
        }
    }
    ...
    @Override
    public void showParamConfDialog() {
    ...
    public void onFragmentResult(int requestCode, int resultCode, Bundle data) {
        ...
        Log.i(TAG,"onFragmentResult requestCode="+requestCode);
            mPresenter.reInitBT();
            mPresenter.initBTListener();
            if(requestCode == REQ_FRAGMENT_WIFICFG && resultCode == RESULT_OK){
                if(data != null){
