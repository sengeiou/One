...
public class PlayActionAdapter extends BaseQuickAdapter<ActionData, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, ActionData item) {
        ...
        ImageView ivPlayPause  = helper.getView(R.id.iv_play_pause_icon);
        ...
        //处理暂停状态图标显示
        if(PlayActionManger.getInstance().getCurrentPlayActionName().equals(item.getActionName()) && PlayActionManger.getInstance().getPlayState() == PlayActionManger.PAUSE ){
            ivPlayPause.setVisibility(View.VISIBLE);
            ivWhite.setVisibility(View.VISIBLE);
        }else {
            ivPlayPause.setVisibility(View.INVISIBLE);
            if(PlayActionManger.getInstance().getPlayState() != PlayActionManger.PLAYING){
                ivWhite.setVisibility(View.INVISIBLE);
            }
