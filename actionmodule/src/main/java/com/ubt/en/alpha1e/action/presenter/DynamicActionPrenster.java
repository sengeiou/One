...
import com.ubt.en.alpha1e.action.model.request.DeleteActionRequest;
import com.ubt.en.alpha1e.action.model.request.GetMessageListRequest;
...
public class DynamicActionPrenster extends BasePresenterImpl<DynamicActionContract.View> implements DynamicActionContract.Presenter {
    private static final String TAG = DynamicActionPrenster.class.getSimpleName();
        ...
        ViseHttp.POST(ActionHttpEntity.ACTION_DYNAMIC_LIST)
                ...
                .request(new ACallback<String>() {
                    ...
                    public void onSuccess(String response) {
                        ViseLog.d( "getDynamicData==" + response);
                        ...
                        if (baseResponseModel.status) {
                            if (mView != null) {
                                ViseLog.d( "getDynamicData.models==" + baseResponseModel.models);
                    ...
                    public void onFail(int i, String s) {
                        ViseLog.d( " getNoticeData onError:" + s);
                        if (mView != null) {
                            mView.setDynamicData(false, type, null);
                        }
        ...
        if (downloadProgressInfo.status == 1) {//正在下载
            ...
            for (int i = 0; i < mDynamicActionModels.size(); i++) {
                if (mDynamicActionModels.get(i).getActionStatu() == 1) {
                    ViseLog.d("praseDownloadData", "actionName==" + mDynamicActionModels.get(i));
    ...
    public void playAction(Context context, int position, List<DynamicActionModel> mDynamicActionModels) {
        ...
        ViseLog.d("", "actionName==" + dynamicActionModel.getActionName() + "  actionStatu===" + dynamicActionModel.getActionStatu());
        if (actionStatu == 0) {
            if (dynamicActionModel.isDownload()) {//已经下载
                for (int i = 0; i < mDynamicActionModels.size(); i++) {
                    if (mDynamicActionModels.get(i).getActionStatu() == 1) {
                        ViseLog.d("DynamicActionFragment", "actionName==" + mDynamicActionModels.get(i));
                        mDynamicActionModels.get(i).setActionStatu(0);
                    }
                }
