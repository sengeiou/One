 ...
 * @author wmma
 */
public class CreateAccountLastFragment extends  MVPBaseFragment<RegisterContract.View, RegisterPresenter> implements RegisterContract.View {
...
//    @BindView(R2.id.tv_service_privacy) TextView tvServicePrivacy;
    ...
    public static CreateAccountLastFragment newInstance(){
        return new CreateAccountLastFragment();
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.login_fragment_create_last_account,container, false);
...
//                btnSignUp.setEnabled(false);
    ...
    @OnClick({ R2.id.iv_clear_password, R2.id.iv_show_password, R2.id.btn_send_security,R2.id.btn_sign_up,R2.id.cl_create_account})
    public void onClickView(View view){
        int i = view.getId();
        if (i == R.id.iv_clear_password) {
            edtPassword.setText("");
...
//            }
    ...
    public void signUpSuccess() {
        ...
        if(mPresenter != null) {
            String userName = SPUtils.getInstance().getString(Constant1E.SP_USER_NAME);
            String sex = SPUtils.getInstance().getString(Constant1E.SP_USER_SEX);
            String age = SPUtils.getInstance().getString(Constant1E.SP_USER_AGE);
            mPresenter.updateUserInfo(userName,sex, age);
        }
//        BaseLoadingDialog.dismiss(getActivity());
//        start(CreateAccountSuccessFragment.newInstance());
    ...
    public void updateUserInfoSuccess(boolean success) {
        if(success) {
            ...
            ToastUtils.showShort("update user info failed");
        }
    }
