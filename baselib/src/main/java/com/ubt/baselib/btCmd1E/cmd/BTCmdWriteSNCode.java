...
public class BTCmdWriteSNCode extends BaseBTReq {
    byte    cmd = BTCmd.READ_SN_CODE;
    byte[] parm = {0x00};

    public BTCmdWriteSNCode(byte[] SNCode) {
        parm = new byte[SNCode.length+1];
        parm[0] = (byte) 0x01;
        System.arraycopy(parm, 1, SNCode, 0, SNCode.length);
        initReq(cmd, parm);
    }
}
