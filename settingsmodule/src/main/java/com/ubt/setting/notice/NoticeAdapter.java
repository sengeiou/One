...
public class NoticeAdapter extends BaseQuickAdapter<NoticeModel, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, NoticeModel item) {
        ...
        if (item.getType().equals("1")) {
            title = "通知消息";
            ((ImageView) helper.getView(R.id.iv_right_icon)).setImageResource(R.drawable.setting_ic_setting_system_push);
        } else if (item.getType().equals("2")) {
            title = "活动消息";
            ((ImageView) helper.getView(R.id.iv_right_icon)).setImageResource(R.drawable.setting_ic_notice_activity);
        }else if (item.getType().equals("3")) {
            title = "帖子消息";
            ((ImageView) helper.getView(R.id.iv_right_icon)).setImageResource(R.drawable.setting_ic_notice_newaction);
        }
